...
public class DetailScreen extends AppCompatActivity implements ViewPagerEx.OnPageChangeListener, BaseSliderView.OnSliderClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        data = (News.Data) getIntent().getSerializableExtra("newsDetails");

        txt_tag.setText(data.getKeywords().toUpperCase());
        txt_title.setText(data.getName().trim());
        txt_name.setText(data.getUpload_by());
        txt_date.setText(data.getPdate());
        txt_desc.setText(HtmlCompat.fromHtml(data.getDescription().trim(), 0));
        txt_desc.setMovementMethod(LinkMovementMethod.getInstance());
        if (data.getIsbookmark().equalsIgnoreCase("0")) {
...
//                .into(img_profile);
        txt_tag.setOnClickListener(view -> Search.gotoSearchPage(DetailScreen.this, "", data.getCid(), "", "", "", data.getKeywords()));
        img_share.setOnClickListener(view -> {
            String shareBody = Constant.get_sp(getApplicationContext(), Constant.Postsharemsg) + "\n\n" + HtmlCompat.fromHtml(data.getDescription().substring(0, Math.min(data.getDescription().length(), Constant.ShareDescWords)), HtmlCompat.FROM_HTML_MODE_COMPACT);
            Constant.shareImage(getApplicationContext(), shareBody, Constant.POST + data.getUp_pro_img(), null);
        });
        img_back.setOnClickListener(view -> finish());
        imgbookmark.setOnClickListener(view -> {
            Constant.saveBookmark(DetailScreen.this, data.getId(), Constant.get_sp(getApplicationContext(), "mobile"), "");
            if (data.getIsbookmark().equalsIgnoreCase("0")) {
                imgbookmark.setImageResource(R.drawable.ic_baseline_bookmark_24);
    ...
    public void getOtherStories() {
        pb.show();
        final Call<News> getUserInfoVoCall = apiListeners.OtherStories("", data.getCid(), "", "", "", Constant.get_sp(DetailScreen.this, "mobile"));
    ...
    public void getSliders() {
        pb.show();
        final Call<NewsGallery> getUserInfoVoCall = apiListeners.NewsGallery(data.getId());
        ...
        getUserInfoVoCall.enqueue(new Callback<NewsGallery>() {
            ...
            public void onResponse(Call<NewsGallery> call, Response<NewsGallery> response) {
                ...
                if (response.body() != null) {
                    if (response.body().getSuccess()) {
                        ar_sliders.add(data.getUp_pro_img());
